...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    await user.save({ validateBeforeSave: false }); // saved on db
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log("Req files obj:", req.files);
  ...
  console.log("avtart response:", avatar);

  if (!avatar) {
    throw new ApiError(400, "Avatar file is required to uplopad");
  }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (incomingRequestToken !== user?.refreshTokens) {
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(200, "Error while uploading on cover image");
  }
